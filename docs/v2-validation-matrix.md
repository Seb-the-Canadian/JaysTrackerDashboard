...
### Resolved in later passes (pass-3 follow-ups)

| # | Finding | Resolution |
|---|---|---|
| V16 | Player card names truncate at 1024px (21/26) and 760px (21/26); 600px and 480px ellipsized at 26/26. The grid's 4-col breakpoint reached down to 980px even though "Vladimir Guerrero Jr." needs ~290px per card to display fully. | Adjusted breakpoints in `players.css` to favor wider cards: 4-col >1200px, 3-col 880–1200, 2-col 620–880, 1-col <620. Plus rank-badge compression: at ≤1200px viewport the " %ile" suffix is hidden via `.pc-stat i small { display: none }` — saves ~25px back to the name column while keeping the ordinal ("61st") readable in context. Probe sweep confirms 0/26 truncate at every tested width (1280 / 1100 / 1024 / 880 / 760 / 600 / 480 / 375). |
| B5 | `#il-chip` had `aria-label="Open injury list"` but `hookIlChip` was a no-op — false affordance. Screen readers announced "Open injury list, button" and Enter did nothing. | Shipped the popover: click the chip → drops below it, lists each player with status badge + ETA note. Esc / click-outside / re-click close; focus returns to chip on Esc. `aria-expanded` + `aria-haspopup="dialog"` + `aria-controls="il-popover"` wired. Re-renders on `jt-data-refresh` event when the next daily refresh lands while the popover is open. Closes the dead-control gap AND the related #28 bug (filter "Reassigned to Minors" out of the list) in one stroke — `filteredInjuries(data)` in `render.js` feeds both `#il-count` and the popover. |
