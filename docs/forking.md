...
| `brand_mark` | **(v1 only)** Single character in the v1 brand box. v2 uses a fixed baseball-diamond mark (team-neutral `--ink`-on-page infield) and does **not** read this field. | Usually first letter of team |
| `analyst_notes_enabled` | If `false`, the hand-authored analyst voice (`notes.json`: overview narrative, per-player/injury notes, team strengths, pitch notes) and the "Analyst voice: Nd old" chip are **not rendered** — only verifiable machine facts + attributed RSS show. Omit or set `true` to enable. Pulled in this repo pending a source-traceable, non-stale voice. | `false` (this fork) |
